...
class VStructureClass:
    ...
    def rotate_vectors(self):
        temp_x = self.x_cg_list
        temp_y = self.y_cg_list
        temp_z = self.z_cg_list
        self.z_cg_list = temp_y
        self.x_cg_list = temp_x
        self.y_cg_list = temp_z

    def approximate_boom_area(self):
        for i in range(0, self.span):
            area_list = []

            start_values = [0, 0.5 * self.x_fs, 0.5 * (self.x_fs + self.x_ms), 0.5 * (self.x_ms + self.x_rs), 0.5 * (1 + self.x_rs)]
            end_values = [0.5 * self.x_fs, 0.5 * (self.x_fs +self.x_ms), 0.5 * (self.x_ms + self.x_rs), 0.5 * (1 + self.x_rs), 1]
    ...
    def estimate_x_cg(self):
        ...
        x_lengths = []
        i = self.x_le
        while i < self.x_le+self.chord_length:
        ...
        cg_x_function = interpolate.interp1d(x_diffs, x_lengths,fill_value="extrapolate")
    ...
    def estimate_y_cg(self):
        y_diffs = []
        y_lengths = []
        i = -self.t_on_c * self.chord_length / 2
        while i < self.t_on_c * self.chord_length / 2:
            pre_pivot_sum_of_moments = 0
            post_pivot_sum_of_moments = 0
            for l in self.wings_point_mass_list:
                for k in l:
    ...
    def calculate_mass_moments(self):
        ...
        val_1 = self.root_chord
        val_2 = ((self.span * tan(self.sweep_angle)) / 2)
        val_3 = self.tip_chord + ((self.span * tan(self.sweep_angle)) / 2)
        list_value = [val_1, val_2, val_3]
        c_c = max(val_1, val_2, val_3)
        c_a = min(val_1, val_2, val_3)
        list_value.remove(c_c)
        list_value.remove(c_a)
        c_b = list_value[0]

        weight_per_chord = (self.mass) / (0.5 * (-c_a + c_b + c_c))
        I = ((weight_per_chord) / 12) * (-c_a ** 3 + c_b ** 3 + c_b * c_c ** 2 + c_c * c_b ** 2 + c_c ** 3)
        sigma_m = 0
        sigma_m_x = 0
        for point_list in self.wings_point_mass_list:
            for point_mass in point_list:
                sigma_m += point_mass.mass
                sigma_m_x += (point_mass.mass * point_mass.x_cg)

        v_plot_constant = [0.55, 0.58, 0.61, 0.70, 0.90]
        k_v = [0.15, 0.25, 0.3, 0.5, 0.95]
        k_v_function = interpolate.interp1d(v_plot_constant, k_v, kind="linear", fill_value="extrapolate")

        #########centroidal##############################
        I_centroid_y = (0.771) * (I - (sigma_m_x / sigma_m))
        constant = (self.cg_y) / (
                (self.span / 6) * ((self.root_chord + 2 * self.tip_chord) / (self.root_chord + self.tip_chord)))
        I_centroid_x = ((self.mass * (self.span ** 2) * k_v_function(constant)) / 24) * (
                (self.root_chord + 2 * self.tip_chord) / (self.root_chord + self.tip_chord))
        I_centroid_z = I_centroid_x + I_centroid_y
        self.Iyy = I_centroid_y
        self.Ixx = I_centroid_x
        self.Izz = I_centroid_z
        ########################################################
