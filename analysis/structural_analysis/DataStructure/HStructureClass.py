...
class HStructureClass:
    ...
    def estimate_z_cg(self):
        ...
        while i < (self.t_on_c * self.chord_length)/2:
            ...
            for l in self.wings_point_mass_list:
                for k in l:
                    if k.z_cg < i:
                        pre_pivot_sum_of_moments += (k.mass * abs(i-k.z_cg))
                    else:
                        post_pivot_sum_of_moments += (k.mass *abs(i-k.z_cg))
    ...
    def calculate_mass_moments(self):
            ...
            h_plot_constant = [0.78, 0.85, 0.90, 1.00, 1.10, 1.20]
            k_h = [0.5, 0.6, 0.7, 0.8, 1.0, 1.2]
            k_h_function = interpolate.interp1d(h_plot_constant, k_h, kind="linear", fill_value="extrapolate")

            #########centroidal##############################
            I_centroid_y = (0.771) * (I - (sigma_m_x / sigma_m))
            constant = (self.cg_y) / ((self.span / 6) * ((self.root_chord + 2 * self.tip_chord) / (self.root_chord + self.tip_chord)))
            I_centroid_x = ((self.mass * (self.span ** 2) * k_h_function(constant)) / 24) * (
