...
class boom_model():
    def __init__(self, config=None, text="", boom_type_="", design_type_=""):
        ...
        self.fuselage_profile = "squareProfile"
    ...
    def get_current_loft(self):
        ...
        for (x, w, h, z, profile, index) in zip(sections, width, height, z_position, profiles,
            ...
            sectionElementCenter.set_center(tigl3.geometry.CTiglPoint(x
                                                                      ...
                                                                      , z))
            if w == h and profile == "circularProfile":
                sectionElementCenter.set_area(pi * (w / 2))
            elif w == h and profile == "rectangularProfile" or profile == "squareProfile":
                sectionElementCenter.set_area(w * h)
            else:
                sectionElementCenter.set_width(w)
                sectionElementCenter.set_height(h)

        loft = [self.fuselage.get_loft().shape()]
