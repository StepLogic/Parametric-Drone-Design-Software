...
class DatcomParser(Parser):
    ...
    def t_INPUT_FLOATVECTOR(self, t):
        try:
            ...
            for num in str.split("".join(str_t), ','):
                vector.append(float(num))
            t.value = vector
        except ValueError:
            self.p_error_vector(t)
            t.value = 0
        # print t
        return t
    ...
    def p_error_vector(self, p):
        if p:
            print("(Vector)Syntax error '%s' at line: %d, state: %s" \
                  % (p.value, self.lex.lineno, self.lex.lexstate))
        else:
            print("Syntax error at EOF")
        sys.exit(1)

    def p_error_float(self, p):
        if p:
            print("(Float)Syntax error '%s' at line: %d, state: %s" \
                  % (p.value, self.lex.lineno, self.lex.lexstate))
        else:
            print("Syntax error at EOF")
        sys.exit(1)

    def p_error_int(self, p):
        if p:
            print("(Int)Syntax error '%s' at line: %d, state: %s" \
                  % (p.value, self.lex.lineno, self.lex.lexstate))
        else:
            print("Syntax error at EOF")
        sys.exit(1)
